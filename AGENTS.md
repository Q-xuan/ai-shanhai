...
- **移动端约定**:标签字号按 `1/view.s` 补偿(钳制 0.6–3 倍),保证任何缩放级别可读;触屏用 `matchMedia("(pointer: coarse)")` 判定,点击半径加大、初始视图不强求一屏看全(下限 0.72 缩放);双指捏合缩放在 pointer 事件层手写,注意 `pinchStart` 要存当时的 `getBoundingClientRect`。
